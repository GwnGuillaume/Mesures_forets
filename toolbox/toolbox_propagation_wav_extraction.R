...
extractNoise <- function(filename, channel ='left', am_timeline, total_dur, t0, dur)
{
  ...
  wav_channel <- wav_vector[, ii] / 2^(wavSM4@bit-1)
  # Create time axis
  time_axis <- seq(from = 0, to = length(wav_channel)-1, by = 1) / wavSM4@samp.rate
  # Group time axis and sound pressure in a dataframe
  wav_df <- data.frame(time_axis, wav_channel)
  # Plot
  sig_plot <- plot_ly(wav_df, type = 'scatter', mode = 'lines') %>% 
    add_trace(x = ~time_axis, y = ~wav_channel, name = 'raw signal') %>% 
    layout(showlegend = FALSE)
  options(warn = -1)
  sig_plot <- sig_plot %>% 
    layout(xaxis = list(zerolinecolor = '#ffff', zerolinewidth = 2, gridcolor = 'ffff', title = "Time (s)"), 
  ...
  {
    print(ii)
    time_expansion.left = 1.2  # Timeline expansion (in s)
    # time_expansion.right = 0.1  # Timeline expansion (in s)
    time.clip.left = am_timeline[ii] - time_expansion.left
    time.clip.right = am_timeline[ii]
    wavSM4 = readWave(filename, 
                      from = time.clip.left, 
                      to   = time.clip.right + total_dur,
                      units = 'seconds')
    ...
    wav_channel <- wav_vector[, ii] / 2^(wavSM4@bit-1)
    # Create time axis
    time_axis <- seq(from = time.clip.left, to = time.clip.right, by = 1/wavSM4@samp.rate)
    # Design the filter
