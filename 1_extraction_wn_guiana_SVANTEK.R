...
# Analog to digital conversion (Vpp [peak to peak])
VADC = 2
...
# ############ FILE PARAMETERS
filename = "./data/guiana/SVANTEK/AUDIO29_data/AUDIO29.WAV"
...
# ##### 
# # Read the full wave file
# fullwavSM4 = readWave(filename = filename, from = 0, to = Inf, units = 'seconds')
# 
# # Get left channel signal
# wav_channel <- fullwavSM4@left
# 
# # Convert the values from bit steps to values between -1 and 1
# wav_channel <- wav_channel / 2^(fullwavSM4@bit -1)
# 
# # Create time column
# timeline <- seq(from = 0, to = length(wav_channel)-1, by = 1) / fullwavSM4@samp.rate
# # timeline <- timeline * 1000  # rescale to milliseconds
# 
# # Create dataframes for left and right signals
# wav_df <- data.frame(timeline, wav_channel)
# 
# # Plot signal
# sig_plot <- plot_ly(wav_df) %>% 
#   add_trace(x = ~timeline, y = ~wav_channel, color = ~as.factor(wav_channel), type = 'scatter', mode = 'lines') %>% 
#   layout(xaxis = list(title = "Time (s)", titlefont = "Courier New, monospace"), 
#          yaxis = list(title = "Amplitude", titlefont = "Courier New, monospace"), 
#          showlegend = FALSE)
# saveWidget(sig_plot, file = "./figures/full_wav_sig.html", selfcontained = FALSE, libdir = "lib")
...
for (ii in 1:(length(df_timeline)-1))
{
  ...
  if (SAVE==TRUE)
  {
    dir.create(file.path('./data/', 'psd'), showWarnings = FALSE)
    FREQUENCY = bkg.freq
    save(filename, 
         DISTANCES, FREQUENCY,
         CHANNEL, Nrep,
